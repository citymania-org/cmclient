	...
	void OnDragDrop_Vehicle(Point pt, int widget)
	{
		switch (widget) {
			...
			case WID_GL_LIST_GROUP: { // Matrix group
				...
				Command<CMD_ADD_VEHICLE_GROUP>::Post(STR_ERROR_GROUP_CAN_T_ADD_VEHICLE, new_g == NEW_GROUP ? CcAddVehicleNewGroup : nullptr, new_g, vindex, citymania::_fn_mod || this->grouping == GB_SHARED_ORDERS);
				break;
			}
