		...
		switch (widget) {
			...
			case WID_VD_DECREASE_SERVICING_INTERVAL: { // decrease int
				...
				if (!v->ServiceIntervalIsPercent() && TimerGameEconomy::UsingWallclockUnits()) {
					mod = citymania::_fn_mod ? 1 : 5;
				} else {
					mod = citymania::_fn_mod ? 5 : 10;
				}
...
public:
	...
	void OnClick([[maybe_unused]] Point pt, WidgetID widget, [[maybe_unused]] int click_count) override
	{
		...
		switch (widget) {
			...
			case WID_VV_GOTO_DEPOT: // goto hangar
				Command<CMD_SEND_VEHICLE_TO_DEPOT>::Post(GetCmdSendToDepotMsg(v), v->index, citymania::_fn_mod ? DepotCommandFlag::Service : DepotCommandFlags{}, {});
				break;
	...
	static inline HotkeyList hotkeys{"vehicleview", {
		...
		Hotkey('G', "cm_vehicle_orders", WID_VV_SHOW_ORDERS),
		Hotkey('F', "cm_vehicle_go", WID_VV_START_STOP),
		Hotkey((uint16)0, "cm_vehicle_refit", WID_VV_REFIT),
		Hotkey((uint16)0, "cm_vehicle_clone", WID_VV_CLONE),
	}};
};
